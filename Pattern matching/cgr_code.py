        ...
        for key, value in probabilities.items():
            for char in key:
               ...
               # (in case of RNA )  U = lower right quadrant then (maxX/2 or maxY/2)
                ...
                maxx = maxx / 2
                maxy /= 2
            # chaos array that is 2   and will be showed in graphical representation
...
def graph(chaos_kx, kmer_size, local_file):
    pylab.figure(1)
    ...
    pylab.axis("off")
    plt.savefig('{}.png'.format(local_file), bbox_inches='tight')
    #pylab.show()
    

# method that shows the graph
def CGR(sequence, local_file):
    ...
    #print(chaos_kx)
    graph(chaos_kx, kmer_size, local_file)
